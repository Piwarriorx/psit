--[[
	PI HUB  |  Grow a Chicken Fighter  (v9.0)
	...
	- Anti-AFK toggle (VirtualInputManager – walang character movement)
]]
...
-- ===== ANTI-AFK (VirtualInputManager – walang galaw) =====
task.spawn(function()
	while GEN == _G.HubGen do
		if state.antiAFK then
			pcall(function()
				-- Fake keypress ng "F1" (walang function sa laro)
				VIM:SendKeyEvent(true, Enum.KeyCode.F1, false, game)
				task.wait(0.1)
				VIM:SendKeyEvent(false, Enum.KeyCode.F1, false, game)
			end)
		end
