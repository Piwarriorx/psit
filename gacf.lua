--[[
	PI HUB  |  Grow a Chicken Fighter  (v8.6)
...
local success, err = pcall(function()
	...
	if not okMods then
		warn("[PiHub] Failed to load game modules. GUI will still appear but features may not work.")
	end
	...
	local function addSlider(key, label, order, default, minVal, maxVal)
		...
		drag.MouseButton1Down:Connect(function()
			...
			con = UIS.InputChanged:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
					...
					local rounded = math.floor(relX * 100 + 0.5)  -- FIX: replaced math.round
		...
		slider.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				local relX = math.clamp((input.Position.X - slider.AbsolutePosition.X) / slider.AbsoluteSize.X, 0, 1)
				local rounded = math.floor(relX * 100 + 0.5)  -- FIX
				updateSlider(rounded)
			end
		end)
		return slider
	...
	task.spawn(function()
		while GEN == _G.HubGen do
			...
			if okMods then
				...
				info.Text = (okR and txt) or "Error loading info"
			else
				info.Text = "Modules not loaded – some features may not work."
			end
			task.wait(0.8)
		end
	end)
...
-- ===== ERROR HANDLING =====
if not success then
	warn("[PiHub] Error during execution:", err)
	print("[PiHub] Please check the console for errors and report.")
	-- Try to show a simple error message GUI
	local pg = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")
	local errorGui = Instance.new("ScreenGui")
	errorGui.Name = "PiHubError"
	errorGui.Parent = pg
